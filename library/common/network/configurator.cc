...
void Configurator::refreshDns(envoy_network_t network) {
  if (!dns_cache_.has_value()) {
    Extensions::Common::DynamicForwardProxy::DnsCacheManagerFactoryImpl cache_manager_factory{
        context_};
    dns_cache_ = cache_manager_factory.get()->lookUpCacheByName("base_dns_cache");
  }
  ...
  // flip-flop scenarios.
  if (network != preferred_network_.load() || dns_cache_.value() == nullptr) {
    return;
  }
...
ConfiguratorSharedPtr ConfiguratorHandle::get() {
  return context_.singletonManager().getTyped<Configurator>(
      ...
      [&] { return std::make_shared<Configurator>(context_); });
}
